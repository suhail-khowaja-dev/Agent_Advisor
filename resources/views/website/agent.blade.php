...
@section('content')
    ...
    @elseif (Session::has('authenticate'))
        <script type="text/javascript">
            swal("Un Authorize!", "{{ Session::get('authenticate') }}", "warning");
        </script>
    @elseif (Session::has('contact'))
        <script type="text/javascript">
            swal("Thank you!", "{{ Session::get('contact') }}", "success");
        </script>
    @elseif (Session::has('profile'))
        <script type="text/javascript">
            swal("Profile!", "{{ Session::get('profile') }}", "success");
        </script>
        ...
        <div class="container">
            <div class="headerSection d-flex justify-content-end">
                ...
                <div class="profileInfo d-flex align-items-center">
                    <div class="userImage">
                        <img src="{{ asset('frontend/images/1x/profileimage.png') }}" alt="profileImage" width="100%"
                            height="100%">
                    </div>
                    <div class="userName">
                        <span>
                ...
                <div class="row">
                    <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6">
                        <div class="referclientbox d-flex clientBox">
                            ...
                            <div class="referenceClientText">
                                ...
                                <a class="d-flex" href="{{ route('agent-contact') }}">
                                    <div class="refcA">
                                        Contact Now
                                    </div>
                                    <div class="imgg">
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6">
                        <div class="referclientbox d-flex clientBox">
                            <div class="imgesec4">
                                <div class="iimg">
                                    <img src="{{ asset('frontend/images/1x/agentRefIcon.png') }}" alt="Image"
                                        width="100%" height="100%">
                                </div>
                            </div>
                            <div class="referenceClientText">
                                <h5>
                                    {{ $Refer_page->client_title ?? '' }}
                                </h5>
                                <p>
                                    {!! $Refer_page->client_description ?? '' !!}
                                </p>
                                {{-- <a href="#" class="d-flex">
                                <div class="refcA">
                                    Refer a Client
                    ...
                    <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6">
                        <div class="referclientbox d-flex clientBox">
                            <div class="imgesec2">
                                <div class="iimg">
                                    <img src="{{ asset('frontend/images/1x/agentEditIc.png') }}" alt="Image"
                                        width="100%" height="100%">
                                </div>
                            </div>
                            ...
                            <div class="referenceClientText">
                                ...
                                <a href="{{ route('agent-profile', Auth::user()->slug) }}" class="d-flex">
                                    <div class="refcA">
                                        Edit Profile
                                    </div>
                                    <div class="imgg">
    ...
    <section>
        <div class="container">
                        ...
                        <tbody>
                            @foreach ($agent as $item)
                                @if (isset($item))
                                    @if ($item->status == 0)
                                        <tr>
                                            <td>{{ Str::limit($item->name, 5, $end = '***') }}</td>
                                            <td>{{ Str::limit($item->email, 3, $end = '***') }}</td>
                                            <td>{{ Str::limit($item->phone_number, 2, $end = '***') }}</td>
                                            <td>{{ "$" . $item->price }}</td>
                                            <td>{{ 'N/A' }}</td>
                                            <td>{{ $item->purchase }}</td>
                                            <td>{{ $item->getstate->state ?? '' }},{{ $item->getcity->city ?? '' }}</td>
                                            <td>
                                                <div class="myNewBtn  d-flex align-items-center ">
                                                    <a href="{{ url('status', $item->id) }}"
                                                        ...
                                                        class="modal-remove{{$item->id}}">
                                                        <button class="accept" id="acceptjob">
                                                            Accept
                                                        </button>
                                                    </a>

                                                    <a href="{{ url('decline', $item->id) }}" class="dclinejob">
            ...
            @elseif ($item->status == 1)
                <td><a href="{{ route('client-details', $item->slug) }}" style="color: #22B573">{{ $item->name }}</a>
                </td>
                <input type="hidden" name="client_id" value="{{ $item->id }}">
                <td style="color: #22B573">{{ $item->email }}</td>
                <td style="color: #22B573">{{ $item->phone_number }}</td>
                <td style="color: #22B573">{{ "$" . $item->price }}</td>
                <td>{{ "$" . $item->after_refer_price ?? 'N/A' }}</td>
                <td style="color: #22B573">{{ $item->purchase }}</td>
                <td style="color: #22B573">{{ $item->getstate->state ?? '' }},{{ $item->getcity->city ?? '' }}</td>
                <td>
    ...
    <script>
        function status(id) {
            $("#terms").attr("href", `{{ env('APP_URL') }}/terms/${id}`);
                $("#terms").val(id);
                    $.ajax({
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf_token"]').attr('content')},
                    type: "GET",
                    url :  `{{ env('APP_URL') }}/terms-get/${id}`,
